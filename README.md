...
- `Release` (`.github/workflows/release.yml`) runs on pushes to `main`/`master` and performs semantic-release (currently GitHub-only: tag + changelog + GitHub Release). NPM publish is intentionally disabled until a token is configured.
...
Semantic-release plugins configured (GitHub-only mode):
- `@semantic-release/commit-analyzer` + `@semantic-release/release-notes-generator` (Conventional Commits)
- `@semantic-release/changelog` updates CHANGELOG.md
- `@semantic-release/npm` (configured with `npmPublish: false` so it ONLY updates `package.json` version; no registry publish)
- `@semantic-release/github` creates the GitHub Release (no assets yet)
- `@semantic-release/git` commits updated CHANGELOG + version bump

To enable npm publishing later:
1. Create an npm automation token (npm website → Access Tokens).
2. Add it as repo secret `NPM_TOKEN`.
3. Remove `"npmPublish": false` from `.releaserc.json` (or set it to true).
4. Push a conventional commit (e.g. `chore: enable npm publish`). The next run will publish.

Optional future hardening:
- Set package.json `version` to `0.0.0-development` prior to first release (semantic-release then tags `1.0.0`).
- Add provenance SBOM generation step.

Conventional Commit examples (triggering releases while in GitHub-only mode):
